...
class TestStringMethods(unittest.TestCase):
    ...
    def test_open(self):
        ...
        for channel in channels:
            ...
            self.assertEqual(channel["ydata"][0:4], [-5.121809677944827e58])
            self.assertIsNotNone(uuid)
            self.assertIsNotNone(name)
            self.assertIsNotNone(description)
            self.assertIsNotNone(origin)
            self.assertIsNotNone(comment)
            self.assertIsNotNone(xname)
            self.assertIsNotNone(xunit)
            self.assertIsNotNone(xlen)
            self.assertIsNotNone(yname)
            self.assertIsNotNone(yunit)
            self.assertIsNotNone(ylen)
